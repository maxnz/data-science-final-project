---
title: "Analysis of FIRST Robotics Competition Team Performance"
...
We got all of the match data from [The Blue Alliance](thebluealliance.com), a website with team and match data for the FIRST Robotics Competition.
They have a public API that we were able to utilize to pull the data.

### Scraping Match Data

The match data in The Blue Alliance is split up by event.
...
We then use `spread()` to put the team name in a column with the name `1`, `2`, `3`, or `4`, based on that column we just created.
Last, we either rename the column from `1` to `blue_alliance_1` if the column `1` exists, otherwise put `NA` into `blue_alliance_1`, repeating for `blue_alliance_2`, `blue_alliance_3` and `blue_alliance_4`.
...
To combine all the events for a year into one data frame, we use `bind_rows()`, with an empty data frame at the beginning.
...
The 140,386 matches we have data for take up 2.6 MB to store when compressed into a `.rds` file, compared to 15.6 MB if uncompressed in a `.csv`.

After we read in the `.rds`, we do a little more data manipulation to denotes postseason events separately from the regular season events in the `week` column using `case_when()`, followed by another `mutate()` that converts the week column to a factor.
...
We then remove the `team_alliance` column (using syntax that requires a newer version of `dplyr` than is available on the R Server) because it contains the same data as the `alliance` column that already existed.
